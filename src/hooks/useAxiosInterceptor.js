...
import { ACCESS_TOKEN_EXPIRED, REFRESH_TOKEN_EXPIRED, UNAUTHORIZED_MEMBER } from "@constants/errorcode";
...
const UseAxiosInterceptor = () => {
  ...
  const responseInterceptor = request.interceptors.response.use(
    ...
    (error) => {
      const { detailErrorCode } = error.response.data?.body ? error.response.data.body : error.response.data;
